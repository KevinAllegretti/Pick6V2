...
function deserializeObject(buffer, index, options, isArray = false) {
    ...
    while (!done) {
        ...
        // Represents the key
        const name = isArray ? arrayIndex++ : byte_utils_1.ByteUtils.toUTF8(buffer, index, i);
        ...
        if (elementType === constants.BSON_DATA_STRING) {
            ...
            const oid = byte_utils_1.ByteUtils.allocate(12);
            ...
            const dataview = byte_utils_1.BSONDataView.fromUint8Array(buffer.subarray(index, index + 8));
            const lowBits = buffer[index++] |
                (buffer[index++] << 8) |
                (buffer[index++] << 16) |
                (buffer[index++] << 24);
            ...
            const bytes = byte_utils_1.ByteUtils.allocate(16);
            ...
            if (buffer['slice'] != null) {
                ...
                const _buffer = byte_utils_1.ByteUtils.allocate(binarySize);
                // If we have subtype 2 skip the 4 bytes for the size
                if (subType === binary_1.Binary.SUBTYPE_BYTE_ARRAY) {
                    binarySize =
                        buffer[index++] |
            ...
            if (i >= buffer.length)
                throw new error_1.BSONError('Bad BSON Document: illegal CString');
            // Return the C string
            const source = byte_utils_1.ByteUtils.toUTF8(buffer, index, i);
            // Create the regexp
            index = i + 1;
            // Get the start search index
            i = index;
            ...
            if (i >= buffer.length)
                throw new error_1.BSONError('Bad BSON Document: illegal CString');
            // Return the C string
            const regExpOptions = byte_utils_1.ByteUtils.toUTF8(buffer, index, i);
            index = i + 1;
            ...
            const t = buffer[index++] +
                ...
                buffer[index++] * (1 << 16) +
                buffer[index++] * (1 << 24);
            value = new timestamp_1.Timestamp({ i, t });
        }
            ...
            const namespace = byte_utils_1.ByteUtils.toUTF8(buffer, index, index + stringSize - 1);
            // Update parse index position
            index = index + stringSize;
            // Read the oid
            const oidBuffer = byte_utils_1.ByteUtils.allocate(12);
...
function getValidatedString(buffer, start, end, shouldValidateUtf8) {
    const value = byte_utils_1.ByteUtils.toUTF8(buffer, start, end);

...
function serializeNumber(buffer, key, value, index) {
    ...
    const bytes = type === constants.BSON_DATA_INT ? FOUR_BYTE_VIEW_ON_NUMBER : EIGHT_BYTE_VIEW_ON_NUMBER;
    buffer[index++] = type;
    const numberOfWrittenBytes = byte_utils_1.ByteUtils.encodeUTF8Into(buffer, key, index);
...
function serializeObjectId(buffer, key, value, index) {
    ...
    if ((0, utils_1.isUint8Array)(idValue)) {
        for (let i = 0; i < 12; i++) {
            buffer[index++] = idValue[i];
        }
    }
    else {
        throw new error_1.BSONError('object [' + JSON.stringify(value) + '] is not a valid ObjectId');
    }
    // Adjust index
    return index;
}

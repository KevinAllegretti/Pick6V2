...
async function getOneSignalUserId() {
    try {
        addDebugLog('🔍', 'Getting OneSignal user ID...');
        ...
        addDebugLog('🔄', 'No ID found, waiting longer...');
        ...
        addDebugLog('❌', 'No user ID found after all attempts');
        return null;
...
// ===== ONESIGNAL INITIALIZATION WITH AUTO-REGISTRATION =====
async function initializeOneSignal() {
    addDebugLog('🚀', 'Starting OneSignal initialization WITH AUTO-REGISTRATION...');
    ...
    try {
        ...
        await OneSignal.init({
            ...
            autoRegister: true,        // ✅ AUTO-REGISTRATION ENABLED
            autoResubscribe: true,     // ✅ AUTO-RESUBSCRIBE ENABLED
            serviceWorkerParam: {
                scope: '/'
            },
            notifyButton: {
        ...
        addDebugLog('✅', 'OneSignal.init() completed WITH AUTO-REGISTRATION');
        ...
        // Check auto-registration result after a delay
        setTimeout(async () => {
            try {
                ...
                addDebugLog('📊', 'AUTO-REGISTRATION RESULT:', { 
                    userId: onesignalId || 'none created yet',
                    subscribed: isOptedIn,
                    permission: permission
                });
                
                if (onesignalId) {
                    addDebugLog('🎉', 'AUTO-REGISTRATION SUCCESS: User created automatically!');
                } else {
                    addDebugLog('⚠️', 'AUTO-REGISTRATION: User not created yet (may need permission)');
                }
            } catch (error) {
                addDebugLog('❌', 'Error checking auto-registration result:', error.toString());
            }
        }, 5000);
        
        addDebugLog('✅', 'OneSignal initialized with AUTO-REGISTRATION enabled');
        return true;
...
async function handleNotificationToggle() {
    ...
    try {
        // Get current OneSignal state (should exist from auto-registration)
        const onesignalId = await getOneSignalUserId();
        const isOptedIn = await OneSignal.User.PushSubscription.optedIn;
...
async function initOnPageLoad() {
    ...
    try {
        ...
        // Initialize OneSignal WITH AUTO-REGISTRATION
        const initSuccess = await initializeOneSignal();
        
        if (initSuccess) {
            addDebugLog('🎉', 'OneSignal initialized with AUTO-REGISTRATION!');
...
async function waitForOneSignalScript() {
    ...
    return new Promise((resolve, reject) => {
        ...
        const checkScript = () => {
            attempts++;
            addDebugLog('🔍', `Script check attempt ${attempts}/${maxAttempts}`);
            ...
            // Check if it has the v16 init method
            if (typeof OneSignal.init === 'function') {
                addDebugLog('✅', 'OneSignal v16 script loaded successfully');
                resolve();
            } else {
                addDebugLog('❌', 'OneSignal loaded but missing v16 init method');
                if (attempts >= maxAttempts) {
                    reject(new Error('Wrong OneSignal version - v16 init method not found'));
                    return;
                }
...
async function initializeOneSignalV16() {
    addDebugLog('🚀', 'Starting OneSignal v16 initialization...');
    
    try {
        // Step 1: Wait for script to be available
        await waitForOneSignalScript();
        ...
        await OneSignal.init({
            ...
            // Add additional v16 configuration
            ...
            notifyButton: {
                enable: false // We're handling this manually
            }
        });
        
        addDebugLog('✅', 'OneSignal.init() completed');
        
        // Step 3: Wait for User API to be ready
        addDebugLog('⏳', 'Waiting for User API to be ready...');
        await waitForUserAPI();
        ...
        addDebugLog('🔍', 'Verifying initialization...');
        await verifyInitializationFixed();
        
        addDebugLog('🎉', 'OneSignal v16 initialization completed successfully!');
        return true;
...
function setupEventListeners() {
    try {
        ...
        addDebugLog('✅', 'Event listeners set up successfully');
    } catch (error) {
        addDebugLog('⚠️', 'Error setting up event listeners:', error.toString());
    }
}
...
async function verifyInitializationFixed() {
    try {
        // Test basic User API access
        const hasUser = !!OneSignal.User;
        const hasPushSubscription = !!(OneSignal.User && OneSignal.User.PushSubscription);
        
        addDebugLog('📊', 'Initialization verification:', {
            hasUser,
            hasPushSubscription,
            browserPermission: Notification.permission
        });
        
        // Try to get current subscription status (this should work even if not subscribed)
        try {
            const isOptedIn = await OneSignal.User.PushSubscription.optedIn;
            addDebugLog('✅', 'Successfully checked subscription status:', isOptedIn);
        } catch (statusError) {
            addDebugLog('⚠️', 'Could not check subscription status:', statusError.toString());
        }
        
        // Try to get OneSignal ID using the FIXED v16 method
        try {
            const onesignalId = await getOneSignalUserIdV16();
            addDebugLog('✅', 'Successfully accessed OneSignal ID (v16 fixed):', onesignalId || 'null (not subscribed)');
        } catch (idError) {
            addDebugLog('⚠️', 'Could not access OneSignal ID:', idError.toString());
        }
        
        // Check what properties are actually available on OneSignal.User
        if (OneSignal.User) {
            const userProps = Object.getOwnPropertyNames(OneSignal.User);
            addDebugLog('🔍', 'Available OneSignal.User properties:', userProps);
        }
        
        addDebugLog('✅', 'Initialization verification completed');
        
    } catch (error) {
        addDebugLog('❌', 'Verification failed:', error.toString());
        throw error;
    }
}
...
// ===== BACKEND COMMUNICATION =====
async function updateBackendNotificationStatus(username, enabled, onesignalId = null) {
    addDebugLog('🔄', 'Updating backend notification status:', { username, enabled, onesignalId });
...
async function testOneSignalStatusV16() {
    addDebugLog('🧪', 'Testing OneSignal status (v16 fixed)...');
    ...
    try {
        ...
        // Check what's actually available on the User object
        const userProps = Object.getOwnPropertyNames(OneSignal.User);
        addDebugLog('🔍', 'OneSignal.User properties:', userProps);
        
        // Test PushSubscription API
        if (!OneSignal.User.PushSubscription) {
            throw new Error('OneSignal.User.PushSubscription not available');
        }
        ...
        // Test ID access using FIXED v16 method
        const onesignalId = await getOneSignalUserIdV16();
        addDebugLog('🆔', 'Current OneSignal ID (v16 fixed):', onesignalId || 'null');
        
        // Check if we can access onesignalId directly
        try {
            const directId = OneSignal.User.onesignalId;
            addDebugLog('🔑', 'Direct onesignalId access:', directId || 'null');
        } catch (directError) {
            addDebugLog('⚠️', 'Direct onesignalId access failed:', directError.toString());
        }
        
        addDebugLog('🎉', 'All v16 tests passed!');
...
function initializeNotificationToggle() {
    ...
    label.addEventListener('click', function(e) {
        ...
        // Handle the toggle using FIXED function
...
function addMobileTestButtons() {
    ...
    testPanel.appendChild(subBtn);
    testPanel.appendChild(unsubBtn);
...
async function initOnPageLoad() {
    ...
    try {
        ...
        const initSuccess = await initializeOneSignalV16();
        
        if (initSuccess) {
            addDebugLog('🎉', 'OneSignal initialization successful!');
...
async function subscribeToOneSignalWithVAPID() {
    addDebugLog('🔔', 'Starting OneSignal subscription with VAPID key...');
    
    try {
        await waitForOneSignalReady();
        addDebugLog('✅', 'OneSignal ready');
...
async function unsubscribeFromOneSignal() {
    ...
    return new Promise((resolve) => {
        OneSignal.push(async function() {
            try {
                const wasSubscribed = await OneSignal.User.PushSubscription.optedIn;
